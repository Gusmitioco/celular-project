...
export function AdminScreenPricesClient() {
  ...
  useEffect(() => {
    ...
    fetch(`${apiBaseUrl}/api/admin/screen-prices/meta`, { credentials: "include", cache: "no-store" })
      .then((r) => r.json())
      .then((j: any) => {
        if (!alive) return;
  ...
  async function loadRows() {
    ...
    const res = await fetch(`${apiBaseUrl}/api/admin/screen-prices?storeId=${storeId}&modelId=${modelId}`, {
      credentials: "include",
      cache: "no-store",
    });
  ...
  function setAvailability(id: number, next: boolean) {
    ...
    if (!next) {
      // off => clear input; backend will set price 0
      setEditPrice((prev) => ({ ...prev, [id]: "" }));
      return;
    }
    // on => force user to type a price; keep existing string if any
  ...
  async function saveAll() {
    ...
    // Validation: available=true requires price > 0
    ...
    const res = await fetch(`${apiBaseUrl}/api/admin/screen-prices/bulk`, {
      ...
      body: JSON.stringify({ storeId, modelId, items }),
    });
  ...
  if (loadingMeta || !meta) {
    return (
      <div className="surface" style={{ padding: 16 }}>
        <p style={{ margin: 0 }}>Carregando… (se falhar, verifique se está logado no /admin/login)</p>
      </div>
    );
  }
  ...
  return (
    <div className="surface" style={{ padding: 16 }}>
      <div className="grid" style={{ gap: 12 }}>
        ...
        <div className="grid grid-2">
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "end", justifyContent: "flex-end" }}>
            <button
              className="btn"
              onClick={() => loadRows()}
              disabled={storeId === "" || modelId === "" || loadingRows || savingAll}
            >
              Recarregar
            </button>
            <button
              ...
              onClick={saveAll}
              disabled={storeId === "" || modelId === "" || loadingRows || savingAll || dirtyIds.length === 0}
            >
        ...
        <div>
          {storeId === "" || modelId === "" ? (
            <div className="cardMeta">Selecione Loja + Marca + Modelo para editar.</div>
          ) : loadingRows ? (
