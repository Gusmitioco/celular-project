...
export function StorePricesClient() {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/store/prices/meta`, {
      credentials: "include",
      cache: "no-store",
    })
      .then((r) => r.json())
  ...
  async function loadRows() {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/store/prices?modelId=${modelId}`, {
      credentials: "include",
      cache: "no-store",
    });
  ...
  async function saveAll() {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/store/prices/bulk`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
      body: JSON.stringify({ modelId, items, currency: "BRL" }),
    });
      ...
      <div className="grid" style={{ gap: 12 }}>
        ...
        <div className="grid" style={{ gap: 12 }}>
          ...
          <div className="btnRow">
            <button
              className={`btn btnPrimary ${modelId === "" ? "btnDisabled" : ""}`}
              disabled={modelId === ""}
              onClick={loadRows}
              type="button"
            >
            ...
            <button
              className={`btn btnPrimary ${(modelId === "" || dirtyIds.length === 0 || savingAll) ? "btnDisabled" : ""}`}
              disabled={modelId === "" || dirtyIds.length === 0 || savingAll}
              ref={saveAllBtnRef}
              onClick={saveAll}
              type="button"
          ...
          {loadingRows ? (
            ...
            <p className="sub" style={{ marginTop: 10 }}>
              Selecione marca + modelo e clique em “Carregar serviços”.
            </p>
          )}
        </div>
      </div>
