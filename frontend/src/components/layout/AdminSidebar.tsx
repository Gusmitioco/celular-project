...
export function AdminSidebar() {
  const Item = ({ href, label }: { href: string; label: string }) => (
    <Link
      href={href}
      className="block rounded-xl px-3 py-2 text-sm font-semibold text-dracula-text/85 ring-1 ring-transparent transition hover:bg-white/[0.14] hover:ring-white/[0.20]"
    >
      {label}
    </Link>
  );
  ...
  return (
    <aside className="w-full md:w-64">
      <div className="rounded-2xl bg-white/[0.14] p-4 ring-1 ring-white/[0.20]  glass-fix">
